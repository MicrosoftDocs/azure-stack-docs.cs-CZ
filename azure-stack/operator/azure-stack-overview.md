---
title: Přehled Azure Stack | Microsoft Docs
description: Přehled toho, co Azure Stack a jak vám umožňuje spouštět služby Azure ve vašem datovém centru.
...
- **Cloudové aplikace, které splňují různé předpisy**. Vývoj a nasazení aplikací v Azure s plnou flexibilitou pro nasazení místně pomocí Azure Stack pro splnění zákonných požadavků nebo zásad. Nevyžadují se žádné změny kódu. Mezi příklady aplikací patří globální audit, finanční vykazování, obchodování s cizím systémem Exchange, hraní online her a generování sestav výdajů.

- **Model cloudové aplikace v místním**prostředí. Používejte služby, kontejnery, bez serveru a architektury mikroslužeb, abyste mohli aktualizovat a roztáhnout existující aplikace nebo vytvářet nové. Používejte konzistentní DevOps procesy napříč Azure v cloudu a Azure Stack v místním prostředí, abyste urychlili modernizaci aplikací pro základní klíčové aplikace.
...
Architektura Azure Stack vám umožní poskytovat služby Azure na hraničních zařízeních pro vzdálená umístění nebo přerušované připojení, které se odpojí z Internetu. Můžete vytvářet hybridní řešení, která zpracovávají data místně v Azure Stack a pak je agregovat do Azure pro další zpracování a analýzu. A konečně vzhledem k tomu, že Azure Stack je nainstalovaná místně, můžete splnit konkrétní zákonné požadavky nebo zásady, a to díky flexibilitě nasazení cloudových aplikací v místním prostředí beze změny kódu.
...
Azure Stack používá k poskytnutí identit buď Azure AD, nebo Active AD FS. Azure AD je cloudový zprostředkovatel identity pro více tenantů od Microsoftu. Většina hybridních scénářů s nasazeními připojenými k Internetu používá jako úložiště identit službu Azure AD.

Pro odpojená nasazení Azure Stack musíte použít AD FS. Poskytovatelé prostředků Azure Stack a další aplikace fungují podobně jako AD FS nebo Azure AD. Azure Stack obsahuje svou vlastní instanci služby Active Directory a Graph API služby Active Directory.

> [!IMPORTANT]
> Po nasazení nemůžete změnit poskytovatele identity. Pokud chcete použít jiného zprostředkovatele identity, musíte Azure Stack znovu nasadit. Další informace o Azure Stack požadavcích na identitu najdete v článku [Přehled identity pro Azure Stack](azure-stack-identity-overview.md).

## <a name="how-is-azure-stack-managed"></a>Jak se Azure Stack spravuje?

Azure Stack můžete spravovat pomocí portálu pro správu, portálu User Portal nebo [PowerShellu](https://docs.microsoft.com/powershell/azure/azure-stack/overview?view=azurestackps-1.7.1). Portály Azure Stack jsou každou zálohované samostatné instance Azure Resource Manager. **Operátor Azure Stack** využívá portál pro správu ke správě Azure Stack a k provádění akcí, jako je vytváření nabídek tenantů, a udržování stavu a monitorování stavu integrovaného systému. Portál User Portal poskytuje samoobslužné prostředí pro využívání cloudových prostředků, jako jsou virtuální počítače, účty úložiště a webové aplikace.

> Další informace o správě Azure Stack pomocí portálu správce najdete v tématu [rychlý Start k portálu pro správu Azure Stack](azure-stack-manage-portals.md).
...
Pokud je nakonfigurován Azure Stack, **Azure Stack uživatel** spotřebovává služby, které nabízí operátor. Uživatelé můžou zřizovat, monitorovat a spravovat služby, ke kterým se přihlásili, jako jsou webové aplikace, úložiště a virtuální počítače.
...
- **App Service**: [Azure App Service na Azure Stack](azure-stack-app-service-overview.md) je nabídka PaaS Microsoft Azure k dispozici pro Azure Stack. Služba umožňuje vašim interním nebo externím zákazníkům vytvářet webové aplikace, rozhraní API a Azure Functions aplikací pro libovolnou platformu nebo zařízení.
- **SQL Server**: k poskytování databází SQL jako služby Azure Stack použijte [poskytovatele prostředků SQL Server](azure-stack-sql-resource-provider.md) . Až nainstalujete poskytovatele prostředků a připojíte ho k jedné nebo více instancím SQL Server, můžete vy a vaši uživatelé vytvářet databáze pro cloudové nativní aplikace, weby, které používají SQL, a další úlohy, které používají SQL.
- **Server MySQL**: k zveřejnění databází mysql jako Azure Stack služby použijte [poskytovatele prostředků serveru MySQL](azure-stack-mysql-resource-provider-deploy.md) . Poskytovatel prostředků MySQL se spouští jako služba na virtuálním počítači jádra serveru Windows Server 2016.
...
- **Domény selhání**: virtuální počítače umístěné ve skupině dostupnosti se fyzicky izolují od sebe navzájem, jejich rozprostření tak, jak je to možné, do více domén selhání (Azure Stackch uzlů). Pokud dojde k selhání hardwaru, virtuální počítače z neúspěšné domény selhání se restartují v jiných doménách selhání. Budou se uchovávat v samostatných doménách selhání z ostatních virtuálních počítačů, ale ve stejné skupině dostupnosti, pokud je to možné. Když se hardware vrátí zpátky do online režimu, virtuální počítače se znovu vyrovnávají, aby se zachovala vysoká dostupnost.
...
[Porovnejte Azure Stack a globální Azure](compare-azure-azure-stack.md).

[Základy správy](azure-stack-manage-basics.md).

[Rychlý Start: použijte portál pro správu Azure Stack](azure-stack-manage-portals.md).