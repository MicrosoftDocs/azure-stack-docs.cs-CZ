...
ms.locfileid: "68303127"
---
# <a name="azure-stack-diagnostics-tools"></a>Nástroje pro diagnostiku Azure Stack

Azure Stack je rozsáhlá kolekce součástí pracujících společně a vzájemně spolupracuje. Všechny tyto komponenty generují vlastní jedinečné protokoly. Díky tomu mohou diagnostikovat problémy náročný úkol, zejména chyby pocházející z více interakcí Azure Stack komponent.

Naše diagnostické nástroje vám pomůžou zajistit, aby byl mechanismus shromažďování protokolů snadný a efektivní. Následující diagram ukazuje, jak nástroje pro shromažďování protokolů v Azure Stack fungují:
...
Tyto soubory se shromažďují a ukládají do sdílené složky pomocí Kolektor trasování. Pomocí rutiny PowerShellu **Get-AzureStackLog** je pak můžete v případě potřeby shromažďovat.
...
Chcete-li spustit nástroj pro shromažďování protokolů v integrovaném systému, je nutné mít přístup k privilegovanému koncovému bodu (PEP). Tady je ukázkový skript, který můžete spustit pomocí PEP a shromažďovat protokoly v integrovaném systému:
...
* Shromáždí protokoly a uloží je v zadaném Azure Storage kontejneru objektů BLOB. Obecná syntaxe pro tuto operaci je následující:
  ...
  > [!NOTE]
  > Tento postup je užitečný, když otevřete případ s podpora Microsoftu a budete požádáni o nahrání protokolů. I v případě, že nemáte přístup ke sdílené složce SMB z virtuálního počítače ERCS a váš virtuální počítač s ERCS nemá přístup k Internetu, můžete v Azure Stack vytvořit účet Blob Storage pro přenos protokolů a potom pomocí klienta tyto protokoly načíst a odeslat je do Microsoftu.  
...
* Každá role má své protokoly uvnitř jednotlivých souborů zip. V závislosti na velikosti shromážděných protokolů může být role rozdělená do více souborů zip. Pokud pro takovou roli chcete, aby všechny soubory protokolu byly v jedné složce, použijte nástroj, který se může volně rozkomprimovat (například 7zip). Vyberte všechny soubory zip pro roli a vyberte **extrahovat sem**. Tím se v jedné sloučené složce rozbalení všechny soubory protokolu pro tuto roli.
