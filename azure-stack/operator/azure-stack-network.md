...
Centrum Azure Stack se sestavuje pomocí technologie s podporou převzetí služeb při selhání ve Windows serveru 2019 a s prostory přímo Část konfigurace fyzické sítě centra Azure Stack používá k zajištění, aby komunikace s přímým přístupem k prostorům, která je potřeba pro řešení, splňovala požadavky na výkon a škálování. Konfigurace sítě používá třídy přenosů k oddělení komunikací s přímým přístupem, které jsou založené na síti RDMA, od využití sítě pomocí infrastruktury centra Azure Stack a klienta.

> [!NOTE]
> Příští aktualizace centra Azure Stack zahrnuje další třídu provozu. V rámci přípravy na tuto změnu společnost Microsoft doporučuje, abyste se obrátili na výrobce OEM, aby provedl požadované změny síťových přepínačů rozhraní pro vytváření racku. Tato změna mandátu se dá provést buď před, nebo po aktualizaci na další verzi.
...
| Soukromé | Používá se pro síť úložiště, privátní virtuální IP adresy, kontejnery infrastruktury a další interní funkce. Od 1910 se velikost této podsítě mění na/20. Další podrobnosti najdete v části [privátní síť](#private-network) v tomto článku. | /20 | 
