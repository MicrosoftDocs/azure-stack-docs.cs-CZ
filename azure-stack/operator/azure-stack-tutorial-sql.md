---
title: Nabízet vysoce dostupné databáze SQL v centru Azure Stack
...
ms.locfileid: "75813772"
---
# <a name="offer-highly-available-sql-databases"></a>Nabízí vysoce dostupné databáze SQL

Jako operátor centra Azure Stack můžete nakonfigurovat virtuální počítače serveru pro hostování SQL Serverch databází. Po úspěšném vytvoření hostitelského serveru SQL a jeho správě pomocí centra Azure Stack mohou uživatelé, kteří se přihlásili k odběru služeb SQL, snadno vytvořit databáze SQL.
...
> * Vytvoření hostitelského serveru SQL centra Azure Stack
> * Vytvoření vysoce dostupné databáze SQL

Vytvoří se dvě skupina dostupnosti VM SQL Server AlwaysOn a nakonfiguruje se pomocí dostupných položek centra Azure Stack na webu Marketplace. 

Než začnete, ujistěte se, že [poskytovatel prostředků SQL Server](azure-stack-sql-resource-provider-deploy.md) byl úspěšně nainstalován a že jsou k dispozici následující položky na webu Azure Stack hub Marketplace:
...
SQL Server po vytvoření a správné konfiguraci skupiny dostupnosti AlwayOn musí mít operátor centra Azure Stack vytvoření hostitelského serveru SQL centra Azure Stack hub, aby mohli uživatelé vytvořit databáze. 
...
Pomocí služby Vyrovnávání zatížení skupiny dostupnosti SQL AlwaysOn a přihlašovacích údajů pro ověřování SQL pro vyrovnávání zatížení teď může operátor centra Azure Stack [vytvořit hostitelský server SQL pomocí skupiny dostupnosti AlwaysOn serveru SQL](azure-stack-sql-resource-provider-hosting-servers.md#provide-high-availability-using-sql-always-on-availability-groups). 
...
Po vytvoření, nakonfigurování a přidání skupiny dostupnosti SQL AlwaysOn Azure Stack jako hostitelského serveru SQL hub pomocí operátoru centra Azure Stack může uživatel s předplatným, včetně SQL Serverch funkcí databáze, vytvářet databáze SQL. Podpora funkcí AlwaysOn pomocí kroků v této části. 
