...
### <a name="migrating-fully-qualified-cmdlet-invocations"></a>Migrace plně kvalifikovaných volání rutin
...
- Z rutiny `InvoiceName` se odebral parametr `Get-AzConsumptionUsageDetail`.  Skripty budou muset pro práci s fakturou používat jiné parametry identity.

### <a name="azcompute-previously-azurermcompute"></a>Az.Compute (dříve AzureRM.Compute)

- Z vlastnosti `IdentityIds` v objektech `Identity` a `PSVirtualMachine` se odebraly hodnoty `PSVirtualMachineScaleSet`. Skripty už by se při zpracování neměly rozhodovat podle hodnoty tohoto pole.
...
- Z vlastnosti `AutoOSUpgradePolicy` se odebraly vlastnosti `AutomaticOSUpgrade` a `UpgradePolicy`.
...
- Z objektů `PurgeDisabled`, `PSKeyVaultKeyAttributes` a `PSKeyVaultKeyIdentityItem` se odebrala vlastnost `PSKeyVaultSecretAttributes`. Skripty už by se při zpracování neměly rozhodovat podle vlastnosti ```PurgeDisabled```.
...
#### <a name="share-snapshot"></a>Snímek sdílené složky
...
#### <a name="set-blob-tier"></a>Nastavení úrovně objektu blob
