...
Konfigurace hostitelského počítače ASDK pro spouštění z CloudBuilder. vhdx:
  ...
  8. Chcete-li ověřit nastavení spouštění, spusťte příkaz `bcdedit` .
...
V AD FS nasazení se jako zprostředkovatel identity používá výchozí adresářová služba razítek. Výchozí účet pro přihlášení je azurestackadmin@azurestack.local a heslo je nastavené na to, co jste zadali jako součást příkazů pro instalaci PowerShellu.
...
### <a name="asdk-installazurestackpocps1-optional-parameters"></a>ASDK InstallAzureStackPOC.ps1 nepovinných parametrů
...
|Znovu spustit|Volitelné|Pomocí tohoto příznaku znovu spusťte nasazení. Použije se veškerý předchozí vstup. Opětovné zadání dříve zadaných dat není podporováno, protože je generováno několik jedinečných hodnot a použito pro nasazení.|
