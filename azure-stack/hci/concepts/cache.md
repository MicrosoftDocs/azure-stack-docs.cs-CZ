...
Nasazení all-Flash se zaměřuje na maximalizaci výkonu úložiště a nezahrnují pevný disk.

![Diagram znázorňuje všechna nasazení v prostředí Flash, včetně NVMe pro kapacitu, NVMe pro mezipaměť s jednotkou SSD pro kapacitu a SSD pro kapacitu.](media/cache/All-Flash-Deployment-Possibilities.png)

### <a name="hybrid-deployment-possibilities"></a>Možnosti hybridního nasazení

Hybridní nasazení mají za cíl vyvážit výkon a kapacitu nebo maximalizovat kapacitu a zahrnovat pevný disk.

![Diagram znázorňuje hybridní nasazení, včetně NVMe pro ukládání do mezipaměti s pevným diskem pro kapacitu, SSD pro mezipaměť s pevným diskem pro kapacitu a NVMe pro mezipaměť s pevným diskem a SSD pro kapacitu.](media/cache/Hybrid-Deployment-Possibilities.png)

## <a name="cache-drives-are-selected-automatically"></a>Automaticky se vyberou jednotky mezipaměti.

V nasazeních s více typy jednotek Prostory úložiště s přímým přístupem automaticky používá všechny jednotky nejrychlejší typ pro ukládání do mezipaměti. Zbývající jednotky se použijí k ukládání.

Typ "nejrychlejší" je určen podle následující hierarchie.

![Diagram znázorňuje, že se typy disků uspořádají rychleji na pomalejší v pořadí NVMe, SSD a neoznačeného disku, který představuje HDD.](media/cache/Drive-Type-Hierarchy.png)
...
Pokud máte SSD a HDD, bude SSD ukládat do mezipaměti pro HDD.
   ...
   >[!NOTE]
   > Jednotky mezipaměti nepřispívají do clusteru do použitelné kapacity úložiště. Všechna data uložená v mezipaměti se také ukládají jinde, nebo se nacházejí v nepatrných fázích. To znamená, že celková kapacita nezpracovaného úložiště vašeho clusteru je součet pouze vaší jednotky kapacity.
...
Při ukládání do mezipaměti pro pevný disk jsou čtení *i* zápisy ukládány do mezipaměti, aby se zajistila latence typu Flash (často ~ 10xější) pro obě. Mezipaměť pro čtení ukládá poslední a často načtená data pro rychlý přístup a minimalizuje náhodný provoz do HDD. (Vzhledem k prodlevám při hledání a rotaci je doba latence a ztráty způsobená náhodným přístupem k disku velká.) Zápisy jsou ukládány do mezipaměti za účelem zvýšení zátěže a stejně jako dříve, pro sloučení zápisů a opětovného zápisu a minimalizaci kumulativního provozu na kapacitní jednotky.

Prostory úložiště s přímým přístupem implementuje algoritmus, který rozpustí zápisy před rozbalením, aby emuluje vzor vstupně-výstupních operací s diskem, který se jeví jako sekvenční, i když je skutečná vstupně-výstupní operace přicházející z úlohy (například virtuální počítače) náhodná. Tím se maximalizuje IOPS a propustnost do HDD.
...
Pak můžete jednotku mezipaměti nahradit stejně jako jakoukoli jinou náhradu jednotky.
   ...
   >[!NOTE]
   > Možná budete muset vypnout, aby bylo možné bezpečně nahradit NVMe, která je Add-In karta (AIC) nebo M. 2 Form Factor.
...
![Diagram zobrazuje možnosti nasazení, včetně NVMe pro mezipaměť i kapacitu, SSD pro mezipaměť i kapacitu a SSD pro mezipaměť a smíšené jednotky SSD a HDD pro kapacitu.](media/cache/Exotic-Deployment-Possibilities.png)
...
Chcete-li toto chování přepsat, použijte rutinu **set-ClusterStorageSpacesDirect** a parametry **CacheModeSSD** a **-CacheModeHDD** . Parametr **CacheModeSSD** nastaví chování mezipaměti při ukládání do mezipaměti pro jednotku SSD. Parametr **CacheModeHDD** nastaví chování mezipaměti při ukládání do mezipaměti pro pevný disk.
