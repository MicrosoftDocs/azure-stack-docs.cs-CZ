---
title: Správa virtuálních počítačů pomocí Windows PowerShellu – Azure Stack HCL
...
Kompletní referenční dokumentaci pro správu virtuálních počítačů pomocí prostředí PowerShell najdete v tématu [Reference k Hyper-V](/powershell/module/hyper-v/?view=win10-ps).

## <a name="create-a-vm"></a>Vytvoření virtuálního počítače  

`New-VM`Rutina se používá k vytvoření nového virtuálního počítače. Podrobné informace o použití najdete v referenční dokumentaci k [novému virtuálnímu počítači](/powershell/module/hyper-v/new-vm?view=win10-ps) .
...
- **-Switch** je název virtuálního přepínače, který má virtuální počítač používat pro připojení k jiným virtuálním počítačům nebo síti. Získejte název virtuálního přepínače pomocí [Get-VMSwitch](/powershell/module/hyper-v/get-vmswitch?view=win10-ps). Příklad:  
...
Pokud chcete vytvořit virtuální počítač s novým virtuálním diskem, který se spustí s imagí operačního systému, přečtěte si příklad PowerShellu v tématu [Vytvoření virtuálního počítače návod pro Hyper-V ve Windows 10](/virtualization/hyper-v-on-windows/quick-start/create-virtual-machine).  
...
Následující příklad vrátí seznam všech spuštěných virtuálních počítačů na serveru přidáním filtru pomocí `Where-Object` příkazu. Další informace najdete v tématu [použití dokumentace Where-Object](/previous-versions/windows/it-pro/windows-powershell-1.0/ee177028(v=technet.10)) .
...
Pomocí `Start-VM` příkazů a `Stop-VM` Spusťte nebo zastavte virtuální počítač. Podrobné informace najdete v dokumentaci [Start-VM](/powershell/module/hyper-v/start-vm?view=win10-ps) a [Stop-VM](/powershell/module/hyper-v/stop-vm?view=win10-ps) reference.
...
`Move-VM`Rutina přesune virtuální počítač na jiný server. Další informace najdete v referenční dokumentaci k [Move-VM](/powershell/module/hyper-v/move-vm?view=win10-ps) .
...
`Import-VM` `Export-VM` Rutiny a importují a exportují virtuální počítač. Následující příklad ukazuje několik příkladů. Další informace najdete v dokumentaci [Import-VM](/powershell/module/hyper-v/import-vm?view=win10-ps)  a [Export-VM](/powershell/module/hyper-v/export-vm?view=win10-ps) reference.
...
`Rename-VM`Rutina slouží k přejmenování virtuálního počítače. Podrobné informace najdete v dokumentaci k [přejmenování-VM](/powershell/module/hyper-v/rename-vm?view=win10-ps) .
...
`Checkpoint-VM`Rutina se používá k vytvoření kontrolního bodu pro virtuální počítač. Podrobné informace najdete v referenční dokumentaci k [kontrolnímu bodu virtuálního počítače](/powershell/module/hyper-v/checkpoint-vm?view=win10-ps) .
...
`New-VHD`Rutina se používá k vytvoření nového virtuálního pevného disku pro virtuální počítač. Podrobné informace o tom, jak ho používat, najdete v referenční dokumentaci k [novému virtuálnímu](/powershell/module/hyper-v/new-vhd?view=win10-ps) pevnému disku.
...
`New-VMSwitch`Rutina se používá pro nový virtuální přepínač na hostiteli virtuálního počítače. Podrobné informace o tom, jak ho používat, najdete v referenční dokumentaci k [New-VMSwitch](/powershell/module/hyper-v/new-vmswitch?view=win10-ps) .
...
`Set-VMMemory`Rutina se používá ke konfiguraci paměti virtuálního počítače. Podrobné informace o tom, jak ho používat, najdete v referenční dokumentaci k [set-VMMemory](/powershell/module/hyper-v/set-vmmemory?view=win10-ps) .
...
`Set-VMProcessor`Rutina se používá ke konfiguraci virtuálních procesorů pro virtuální počítač. Podrobné informace o tom, jak ho používat, najdete v referenční dokumentaci k [set-VMProcessor](/powershell/module/hyper-v/set-vmprocessor?view=win10-ps) .
...
Virtuální počítače taky můžete vytvářet a spravovat pomocí centra pro správu Windows. Další informace najdete v [centru pro správu systému Windows](vm.md).
